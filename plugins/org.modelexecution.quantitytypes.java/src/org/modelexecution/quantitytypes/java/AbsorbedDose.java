...
public class AbsorbedDose extends Quantity {
	...
	public AbsorbedDose(Quantity q) {
		value = q.value.clone();
		if (!checkUnit(q.unit))
			throw new RuntimeException("Invalid Unit for creating a AbsorbedDose");
	...
	public AbsorbedDose(UReal u, Unit unit) {
		value = u.clone();
		if (!checkUnit(unit))
			throw new RuntimeException("Invalid Unit for creating an AbsorbedDose");
	...
	public AbsorbedDose(String x) { // creates a AbsorbedDose from a string
									// representing a real, with u=0.
		value = new UReal(x);
		unit = new Unit(DerivedUnits.Gray);
	}

	public AbsorbedDose(String x, String u) { // creates a AbsorbedDose from two
												// strings representing (x,u).
		value = new UReal(x, u);
		unit = new Unit(DerivedUnits.Gray);
	}

	public AbsorbedDose(String x, String u, Unit unit) { // creates a
															// AbsorbedDose from
															// two strings
															// representing
															// (x,u).
	...
	public AbsorbedDose add(AbsorbedDose r) { // only works if compatible units
												// && operand has no offset
		return new AbsorbedDose(super.add(r));
	}

	public AbsorbedDose minus(AbsorbedDose r) { // only works if compatible
												// units. You can subtract 2
												// units with offsets, but it
												// returns a DeltaUnit (without
												// offset)
		return new AbsorbedDose(super.minus(r));
	}

	public EnergyAndWork mult(Mass r) { // both values and units are multiplied.
										// No offsets allowed in any of the
										// units
		return new EnergyAndWork(super.mult(r));
	}

	public KinematicViscosity mult(Time r) { // both values and units are
												// multiplied. No offsets
												// allowed in any of the units
		return new KinematicViscosity(super.mult(r));
	}

	public AbsorbedDoseRate mult(Activity r) { // both values and units are
												// multiplied. No offsets
												// allowed in any of the units
		return new AbsorbedDoseRate(super.mult(r));
	}

	public EnergyDensity mult(Density r) { // both values and units are
											// multiplied. No offsets allowed in
											// any of the units
		return new EnergyDensity(super.mult(r));
	}

	public AbsorbedDoseRate mult(Frequency r) { // both values and units are
												// multiplied. No offsets
												// allowed in any of the units
		return new AbsorbedDoseRate(super.mult(r));
	}

	public HeatFlowRate mult(MassPerUnitTime r) { // both values and units are
													// multiplied. No offsets
	...
	public EnergyPerUnitArea mult(MassPerUnitArea r) { // both values and units
														// are multiplied. No
														// offsets allowed in
														// any of the units
	...
	public Force mult(MassPerUnitLength r) { // both values and units are
												// multiplied. No offsets
												// allowed in any of the units
		return new Force(super.mult(r));
	}

	public InversePermittivity mult(Permeability r) { // both values and units
														// are multiplied. No
														// offsets allowed in
														// any of the units
	...
	public Area mult(TimeSquared r) { // both values and units are multiplied.
										// No offsets allowed in any of the
										// units
		return new Area(super.mult(r));
	}

	public LinearAcceleration divideBy(Length r) { // both values and units are
													// divided. No offsets
													// allowed in any of the
													// units
	...
	public AbsorbedDoseRate divideBy(Time r) { // both values and units are
												// divided. No offsets allowed
												// in any of the units
		return new AbsorbedDoseRate(super.divideBy(r));
	}

	public SpecificHeatCapacity divideBy(ThermodynamicTemperature r) { // both
																		// values
																		// and
																		// units
																		// are
	...
	public Time divideBy(AbsorbedDoseRate r) { // both values and units are
												// divided. No offsets allowed
												// in any of the units
		return new Time(super.divideBy(r));
	}

	public KinematicViscosity divideBy(Activity r) { // both values and units
														// are divided. No
														// offsets allowed in
														// any of the units
	...
	public MolarEnergy divideBy(AmountOfSubstancePerUnitMass r) { // both values
																	// and units
																	// are
																	// divided.
																	// No
	...
	public Exposure divideBy(ElectromotiveForce r) { // both values and units
														// are divided. No
														// offsets allowed in
														// any of the units
	...
	public ElectromotiveForce divideBy(Exposure r) { // both values and units
														// are divided. No
														// offsets allowed in
														// any of the units
	...
	public KinematicViscosity divideBy(Frequency r) { // both values and units
														// are divided. No
														// offsets allowed in
														// any of the units
		return new KinematicViscosity(super.divideBy(r));
	...
	public MassPerUnitLength divideBy(GravitationalAttraction r) { // both
																	// values
																	// and units
																	// are
																	// divided.
	...
	public Activity divideBy(KinematicViscosity r) { // both values and units
														// are divided. No
														// offsets allowed in
														// any of the units
	...
	public Length divideBy(LinearAcceleration r) { // both values and units are
													// divided. No offsets
													// allowed in any of the
													// units
	...
	public LinearVelocity divideBy(LinearVelocity r) { // both values and units
														// are divided. No
														// offsets allowed in
														// any of the units
	...
	public GravitationalAttraction divideBy(MassPerUnitLength r) { // both
																	// values
																	// and units
																	// are
																	// divided.
	...
	public AmountOfSubstancePerUnitMass divideBy(MolarEnergy r) { // both values
																	// and units
																	// are
																	// divided.
																	// No
	...
	public ThermodynamicTemperature divideBy(SpecificHeatCapacity r) { // both
																		// values
																		// and
																		// units
																		// are
	...
	public Activity divideBy(ThermalDiffusivity r) { // both values and units
														// are divided. No
														// offsets allowed in
														// any of the units
		return new Activity(super.divideBy(r));
	...
	public Length divideBy(ThrustToMassRatio r) { // both values and units are
													// divided. No offsets
													// allowed in any of the
													// units
		return new Length(super.divideBy(r));
	...
	public UReal divideBy(AbsorbedDose r) { // This operation converts first
	...
	public AbsorbedDose floor() { // returns (i,u) with i the largest int such
									// that (i,u)<=(x,u) -- units maintained
		return new AbsorbedDose(Math.floor(this.getX()), this.getU(), this.getUnits());
	}

	public AbsorbedDose round() { // returns (i,u) with i the closest int to x
									// -- units maintained
