...
public class AreaTemperature extends Quantity {
	...
	public AreaTemperature(Quantity q){
    	value = q.value.clone();
    	if (!checkUnit(q.unit)) throw new RuntimeException("Invalid Unit for creating an AreaTemperature");
    ...
    public AreaTemperature(String x) { //creates an AreaTemperature from a string representing a real, with u=0.
    	value = new UReal(x);
        unit = new Unit(DerivedUnits.SquareMeterKelvin);
   }
    
    public AreaTemperature(String x, String u) { //creates an AreaTemperature from two strings representing (x,u).
    	value = new UReal(x,u);
        unit = new Unit(DerivedUnits.SquareMeterKelvin);
   }
   
    public AreaTemperature(String x, String u, Unit unit) { //creates an Angle from two strings representing (x,u).
    	value = new UReal(x,u);
