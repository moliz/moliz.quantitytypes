...
public LuminousIntensity(String x) { //creates a LuminousIntensity from a string representing a real, with u=0.
value = new UReal(x);
unit = new Unit(BaseUnits.Candela);
}
public LuminousIntensity(String x, String u) { //creates a LuminousIntensity from two strings representing (x,u).
value = new UReal(x,u);
unit = new Unit(BaseUnits.Candela);
}
public LuminousIntensity(String x, String u, Unit unit) { //creates a LuminousIntensity from two strings representing (x,u).
value = new UReal(x,u);
if (!checkUnit(unit)) throw new RuntimeException("Invalid Unit for creating a LuminousIntensity");
this.unit = new Unit (unit);
}
...
public LuminousFlux mult(Angle r) { //both values and units are multiplied. No offsets allowed in any of the units
return new LuminousFlux(super.mult(r));
}
public Luminance divideBy(Area r) { //both values and units are divided. No offsets allowed in any of the units
return new Luminance(super.divideBy(r));
}
public Area divideBy(Luminance r) { //both values and units are divided. No offsets allowed in any of the units
return new Area(super.divideBy(r));
}
public UReal divideBy(LuminousIntensity r) { //This operation converts first both operands to SI units and then divides)
