...
public class N_UReal implements Cloneable {
	...
	private void fillSample(double s[], double x1, double u1, Distribution dist) {
		...
		case TRIANGULAR:
			...
			break;
		case TRUNCATED:
			double beta = 2 / 3; // ratio between the lengths of the top of the
									// trapezoid and of the base
			for (int i = 0; i < MAXLENGTH; i++) {
	...
	// creates an N_UReal from two numbers representing (x,u), given a
	// distribution
	...
	public N_UReal(String x) { // creates an N_UReal from a string representing
								// a real, with u=0.
	...
	public N_UReal(String x, String u, Distribution dist) {
		// creates an N_UReal from two strings representing (x,u), given a
		// distribution
		this.x = Double.parseDouble(x);
		this.u = Double.parseDouble(u);
	...
	public boolean lessThan(N_UReal r) {
		...
		result = (this.getX() < r.getX()) && ((this.getX() + this.getU()) < (r.getX() - r.getU()));
		return result;
	}
	...
	public N_UReal floor() { // returns (i,u) with i the largest int such that
								// (i,u)<=(x,u)
		double[] s = getSample();
	...
	public String toString() {
		return "(" + x + "," + u + ") [" + sample[0] + "," + sample[1] + "," + sample[2] + "," + sample[3] + ","
				+ sample[4] + "]";
	}
