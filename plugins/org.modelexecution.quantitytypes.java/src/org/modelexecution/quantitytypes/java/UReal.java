...
public class UReal implements Cloneable, Comparable<UReal> {
	...
	public UReal(String x) { // creates an UReal from a string representing a
								// real, with u=0.
		this.x = Double.parseDouble(x);
		this.u = 0.0;
	}

	public UReal(String x, String u) { // creates an UReal from two strings
										// representing (x,u).
		this.x = Double.parseDouble(x);
	...
	public UReal floor() { // returns (i,u) with i the largest int such that
							// (i,u)<=(x,u)
	...
	public UReal min(UReal r) {
		if (r.lessThan(this))
			return new UReal(r.getX(), r.getU());
